...
def alterTable(tableName, tipo, columnF=None, newName=None):
    retorno = ''
    if is_enabled(tableName):
        ...
        with open(path, 'r') as file:
            ...
            content = json.loads(content)
            if tipo == 'ADD':
                content['columnfamilies'].extend(columnF)
                retorno += (
                    f" >> Se han agregado con exito datos a la tabla {tableName}\n")
                with open(path, 'w') as file:
                    json.dump(content, file)
            elif tipo == 'DROP':
                ...
                for item in columnF:
                    if item in content['columnfamilies']:
                        ...
                        for regionkey in content['regions'].keys():
                            for rowkey in content['regions'][regionkey].keys():
                                if item in content['regions'][regionkey][rowkey]:
                    ...
                    # for i in columnF:
                    #     content['columnfamilies'] = content['columnfamilies'].remove(columnF)
                ...
                if os.path.exists(newPath):
                    ...
                    retorno += f' >> la tabla {tableName} a sido renombrada con exito a {newName}'
    else:
        retorno += ' >> ERROR: La lista que mencionas no esta habilitada.\n'
    return retorno
...
def dropTable(tableName):
    ...
    if os.path.exists(path):
        if is_enabled(tableName):
            retorno += (
                f" >> La tabla <{tableName}> no se puede eliminar mientras esté enabled.\n")
        else:
            os.remove(path)
            retorno += (f" >> La tabla <{tableName}> ha sido eliminada correctamente.\n")
    else:
        retorno += (f" >> La tabla <{tableName}> no existe.\n")
...
def dropAll():
    ...
    for archivo in os.listdir('./data'):
        if archivo.endswith(".json"):
            ...
            retorno += (
                f" >> El archivo {archivo} ha sido eliminado correctamente.\n")
...
def Scan(tableName, startrow=None, stoprow=None, columns=None):
    ...
    if is_enabled(tableName):
        ...
        if startrow and stoprow:
            tempRow_last = []
            stR = False
            retorno += ('ROW\t\tCOLUMN\t\t\t\TIMESTAMP\t\t\tVALUE\n')
            for region in content['regions'].keys():
                for rowkey in content['regions'][region]:
                    tempRow_last.append(rowkey)
                    if startrow in tempRow_last[-1]:
                        stR = True
                    if len(tempRow_last) != 1:
                        if stoprow in tempRow_last[-2]:
                            stR = False
                    if stR:
            ...
            retorno += '>> ERROR: Has ingresado startrow pero no stoprow.\n'
        elif not startrow and stoprow:
            retorno += '>> ERROR: Has ingresado stoprow pero no startrow.\n'
        # Mostrar con el filtro de columns
            ...
            for region in content['regions'].keys():
                for rowkey in content['regions'][region]:
                    for i in columns:
                        if i in content['regions'][region][rowkey]:
                            for columnName in content['regions'][region][rowkey][i]:
                                for timestamp in content['regions'][region][rowkey][i][columnName]:
                                    value = content['regions'][region][rowkey][i][columnName][timestamp]
                                    retorno += (
                                        str(rowkey) + '\t' + str(i) + '\t' + str(columnName) + '\t' +
                                        str(timestamp) + '\t' +
                                        str(value) + '\n'
                                    )
...
def Truncate(tableName):
    ...
    retorno += disableTable(tableName)
    retorno += dropTable(tableName)
    retorno += createTable(tableName, colFam)
    retorno += (f'\n >> Truncate exitoso con la tabla {tableName}\n')
    return retorno
